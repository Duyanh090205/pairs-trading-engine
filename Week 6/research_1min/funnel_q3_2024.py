"""
Phễu chọn cặp 1-phút — chạy thật trên Q3/2024 (spec: Week 6/documents/funnel_1min_spec.md)

Các tầng LỌC (tư cách):
  0. Universe: coverage >=95% (spread_summary n_obs), file 5-min đủ bar Q3
  1. Johansen 5-phút (det_order=0, k_ar_diff=12) TẤT CẢ cặp + BH-FDR q=0.05
     (p-value ~ chi2(8) trên trace stat — quy ước Week 4 discovery.py)
  2. Split-sample gate: 2 nửa formation đều vượt cv90
  3. Half-life 1-phút (estimator kháng nhiễu log-ACF slope k=1..10) trong [2, 390] phút

ĐO & XẾP HẠNG (không lọc): biên độ spread (std, p95) vs phí round-trip
(2*(half_A + |beta|*half_B), median Q3 từ spreads_1min THẬT Week 5), số lần cắt
mean/ngày, phân bố beta, corr, số cặp mỗi mã (cờ ETF).

Outputs -> results/funnel_q3_2024/{pairs_all.parquet, survivors_ranked.csv, report.txt}
"""
import json
import os
import sys
...
OUT = ROOT / "Week 6" / "research_1min" / "results" / "funnel_q3_2024"
ALIGNED_PATH = OUT / "aligned_5min.parquet"
...
# ---------------------------------------------------------------- workers
_M = None          # aligned 5-min log-price matrix (numpy), set per worker
_COLS = None
...
def _test_chunk(args):
    """Full-window Johansen + corr for a chunk of (ta, tb) pairs."""
...
def _gate_chunk(pairs):
    """Split-sample gate: both halves must clear cv90."""
    out = []
    for ta, tb in pairs:
        a = _M[:, _COLS[ta]]
        b = _M[:, _COLS[tb]]
...
def main():
    ...
    # ---- Buoc 0: universe + aligned matrix ----
    ...
    for tck in cov_ok:
        f = B5 / f"{tck}.parquet"
        if f.exists():
            s = pd.read_parquet(f)["log_close"].loc[START:END]
            if len(s) > 0:
                series[tck] = s
    M = pd.DataFrame(series)
    expected = int(M.notna().sum().max())
    keep = [c for c in M.columns if M[c].notna().sum() >= MIN_OVERLAP * expected]
    M = M[keep]
    M.to_parquet(ALIGNED_PATH)
    say(f"BUOC 0: universe = {len(keep)} ma (coverage>=95% + du bar Q3; expected {expected} bar 5-min)")

    pairs = list(combinations(keep, 2))
    say(f"        so cap = {len(pairs):,}")
    min_bars = int(MIN_OVERLAP * expected)

    # ---- Buoc 1: Johansen all pairs ----
    say(f"BUOC 1: Johansen 5-min, {N_WORKERS} workers ...")
    ...
    with ProcessPoolExecutor(max_workers=N_WORKERS, initializer=_init_worker,
        ...
        for ci, res in enumerate(ex.map(_test_chunk, chunks, chunksize=4)):
            rows.extend(res)
            if (ci + 1) % 40 == 0:
                say(f"        {min((ci+1)*CHUNK, len(pairs)):,}/{len(pairs):,} cap ({time.time()-t0:.0f}s)")
    df = pd.DataFrame(rows, columns=["ta", "tb", "n_bars", "trace", "pval", "beta", "corr"])
    tested = df["pval"].notna()
    say(f"        test duoc {tested.sum():,} cap (bo {(~tested).sum():,} do overlap/loi)")
    ...
    # ---- Buoc 2: split gate ----
    gate_in = df.loc[df["fdr_pass"], ["ta", "tb"]].itertuples(index=False, name=None)
    gate_pairs = list(gate_in)
    ...
    # ---- Buoc 3: HL 1-min + do luong ----
    surv = df[df["gate_pass"]].copy()
    tickers_needed = sorted(set(surv["ta"]) | set(surv["tb"]))
    say(f"BUOC 3: nap 1-min cho {len(tickers_needed)} ma, do HL/bien do/cat-mean ...")
    l1 = {}
    for tck in tickers_needed:
        s = pd.read_parquet(B1 / f"{tck}.parquet", columns=["close"])["close"].loc[START:END]
        l1[tck] = np.log(s)
    hl_l, xd_l, amp_s, amp_p = [], [], [], []
    for r in surv.itertuples():
        pair_df = pd.concat({"a": l1[r.ta], "b": l1[r.tb]}, axis=1).dropna()
    ...
    # ---- phi round-trip (Q3 median, spread THAT Week 5) ----
    final = surv[surv["hl_band"]].copy()
    cost_tickers = sorted(set(final["ta"]) | set(final["tb"]))
    say(f"        doc phi Q3 (spreads_1min) cho {len(cost_tickers)} ma ...")
    ...
    say("")
    say("================== KET QUA PHEU Q3/2024 ==================")
    say(f"52k? cap test: {tested.sum():,} -> FDR: {n_fdr:,} -> gate: {n_gate:,} -> HL band: {len(final):,}")
    say("")
    b = final["beta"]
    say(f"PHAN BO BETA (survivors): min {b.min():.2f} | p5 {b.quantile(.05):.2f} | median {b.median():.2f} | p95 {b.quantile(.95):.2f} | max {b.max():.2f}")
    say(f"  beta<0: {(b<0).sum()} cap | |beta|>5: {(b.abs()>5).sum()} cap")
    say(f"PHAN BO CORR (survivors): median {final['corr'].median():.2f} | min {final['corr'].min():.2f}")
    say(f"HL: median {final['hl_min'].median():.0f} phut | p5 {final['hl_min'].quantile(.05):.0f} | p95 {final['hl_min'].quantile(.95):.0f}")
    say(f"CAT MEAN/NGAY: median {final['crossings_day'].median():.1f}")
    say(f"RATIO std/cost: >2x: {(final['ratio_std']>=2).sum()} cap | 1-2x: {((final['ratio_std']>=1)&(final['ratio_std']<2)).sum()} | <1x: {(final['ratio_std']<1).sum()}")
    viable = final[final["ratio_std"] >= 2]
    if len(viable):
        mx = pd.concat([viable["half_A_bps"], viable["half_B_bps"]]).max()
        say(f"SPREAD TOI DA con loi (ratio>=2, per-leg half-spread): {mx:.1f} bps")
    cnt = pd.concat([final["ta"], final["tb"]]).value_counts()
    say(f"SO CAP MOI MA (top 10): " + ", ".join(f"{t}:{c}{'(ETF)' if t in ETFS else ''}" for t, c in cnt.head(10).items()))
